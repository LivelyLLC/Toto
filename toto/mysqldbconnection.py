...
class MySQLdbSession(TotoSession):
    ...
    class MySQLdbAccount(TotoAccount):
        ...
        def _load_property(self, *args):
            return self._session._db.get('select ' + ', '.join(args) +
        ...
        def _save_property(self, *args):
            self._session._db.execute('update account set ' + ', '.join(
                ...
                [self[k] for k in args] + [self._session.account_id, ]))

        def __setitem__(self, key, value):
            if key != 'account_id':
                super(MySQLdbSession.MySQLdbAccount, self).__setitem__(key,
                                                                       value)
...
class MySQLdbConnection(DBConnection):
    def create_tables(self, database):
        if not self.db.get('''show tables like "account"'''):
            self.db.execute(
                ''.join(['''CREATE TABLE IF NOT EXISTS `account` (''',
                         self.uuid_account_id and
                         '''`account_id` binary(16) not null,''' or
    ...
    def __init__(self, host, database, username, password,
                 ...
                 session_renew=0, anon_session_renew=0, uuid_account_id=False):
