...
class MessageQueue():
    ...
    def address(self):
        #
        # TODO: Are these a bug? where are the variables below fetched from?
        #
    ...
    def start_listening(self, on_recv=None, port=0, protocol='tcp',
                        interface='*'):
        if self.__thread:
            return
        if on_recv:
        ...
        def listen():
            ...
            if port:
                ...
                self.__port = socket.bind_to_random_port('%s://%s', protocol,
                                                         interface)
            while True:
                try:
                    message = socket.recv()
