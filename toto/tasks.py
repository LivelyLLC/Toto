...
while limiting the number of active jobs. The task queue is designed primarily
for shorter, lightweight jobs. For CPU intensive tasks or tasks that are
expected to run for a long time, look at Toto's worker functionality instead.
"""
...
class TaskQueue():
    ...
    def yield_task(self, fn, *args, **kwargs):
        ...
        to yield the return value from within a function decorated with
        ``@tornado.gen.coroutine`` or ``@tornado.gen.engine``.
        ...
        (result, error), _ = yield Task(
            lambda callback: self.add_task(call, callback))
        if error:
            raise error
        raise Return(result)
