...
class TotoSocketHandler(WebSocketHandler):
    ...
    def configure(cls):
        ...
        open_function = options.socket_opened_method and \
                        options.socket_opened_method.rsplit('.', 1)
        cls._on_open = open_function and getattr(__import__(open_function[0]),
                                                 open_function[1]) or None
        closed_function = options.socket_closed_method and \
                          options.socket_closed_method.rsplit('.', 1)
        cls._on_close = closed_function and getattr(
            __import__(closed_function[0]), closed_function[1]) or None
    ...
    def retrieve_session(self, session_id):
        self.session = self.db_connection.retrieve_session(session_id, None,
                                                           None)
        return self.session
    ...
    def send_message(self, data, message_id=None):
        self.write_message(
            message_id and {'message_id': message_id, 'data': data} or data)
