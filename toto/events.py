"""Toto's event framework is used to allow external events to affect client
requests, or to run scheduled tasks after a specified signal is received.
It can be used to send messages to active requests, even between multiple
server processes. The event framework can also be used outside of Toto to
send messages to running Toto servers.
"""
...
class EventManager():
    ...
    def register_server(self, address):
        """Add a server located at ``address``. This server will now be included
        in the recipient list whenever ``send()`` is called.
        """
    ...
    def remove_server(self, address):
        """Remove the server located at ``address`` from the recipient list for
        all future calls to ``send()``.
        """
    ...
    def refresh_server_queue(self):
        """Reload and shuffle the registered server queue used for round-robin
        load balancing of non-broadcast events.
        """
    ...
    def register_handler(self, event_name, event_handler,
                         run_on_main_loop=False, request_handler=None,
                         persist=False):
        """Register ``event_handler`` to run when ``event_name`` is received.
        Handlers are meant to respond to a single event matching ``event_name``
        only. If ``run_on_main_loop`` is ``True`` the handler will be executed
        on Tornado's main ``IOLoop`` (required if the handler will write to a
        response stream). If ``request_handler`` is set, ``event_handler`` will
        not fire once ``request_handler`` has finished. Set ``persist`` to
        ``True`` to automatically re-queue ``event_handler`` each time it is
        executed.
        """
        if not event_name in self.__handlers:
            self.__handlers[event_name] = set()
        handler_tuple = (event_handler, run_on_main_loop, request_handler,
                         persist)
                    ...
                    for handler in list(handlers):
                        ...
                        try:
                            if handler[2] and handler[2]._finished:
                                continue
                            if handler[1]:
                                (lambda h: IOLoop.instance().add_callback(
                                    lambda: h[0](event_args)))(handler)
                            else:
    ...
    @classmethod
    def instance(cls):
        """Returns the shared instance of ``EventManager``, instantiating on
        the first call.
        """
