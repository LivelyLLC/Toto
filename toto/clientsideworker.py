...
class ClientSideWorkerManager(object):
    ...
    def run_operation(self):
        while self.__operation_queue and self.__worker_queue:
            ...
            self.__workers[worker_id].write_message(
                {'operation_id': operation[0], 'script': operation[1]})
            if (operation[2]):
                self.__operation_queue.append(operation)

    def add_operation(self, operation_id, operation_script,
                      callback_method=None, max_nodes=1, continuous=False):
        nodes = min(max_nodes, len(self.__worker_queue)) or 1
        if callback_method:
            self.__operation_callbacks[operation_id] = callback_method
        for i in xrange(nodes):
            self.__operation_queue.append(
                (operation_id, operation_script, continuous))
        self.run_operation()
...
# These methods allow ClientSideWorkerSocketHandler to be swapped with the
#  bulkier TotoSocketHandler
...
def complete(worker, data):
    ClientSideWorkerManager.instance().finish_operation(worker,
                                                        data['operation_id'],
                                                        data['result'])
...
class ClientSideWorkerSocketHandler(WebSocketHandler):
    ...
    def log_error(self, e):
        if isinstance(e, TotoException):
            ...
            logging.error(
                "TotoException: %s Value: %s" % (ERROR_SERVER, repr(e)))
    ...
    def on_message(self, message_data):
        ...
        ClientSideWorkerManager.instance().finish_operation(self, message[
            'operation_id'], message['result'])
