...
class RedisSession(TotoSession):
    ...
    class RedisAccount(TotoAccount):
        def _load_property(self, *args):
            return dict(zip(args, self._session._db.hmget(
                _account_key(self._session.user_id), args)))

        def _save_property(self, *args):
            self._session._db.hmset(_account_key(self._session.user_id),
    ...
    def refresh(self):
        session_data = self._refresh_cache() or TotoSession.loads(
            self._db.get(_session_key(self.session_id)))
    ...
    def save(self):
        ...
        if not self._save_cache():
            self._db.setex(_session_key(self.session_id),
                           int(self.expires - time()),
                           TotoSession.dumps(self.session_data()))
...
class RedisConnection(DBConnection):
    def __init__(self, host='localhost', port=6379, database=0,
                 ...
                 session_renew=0, anon_session_renew=0):
    ...
    def create_session(self, user_id=None, password=None, verify_password=True):
        ...
        if not user_id:
            user_id = ''
        account_key = _account_key(user_id)
        account = user_id and password and self.db.hmget(account_key, 'user_id',
                                                         'password')
    ...
    def change_password(self, user_id, password, new_password):
        ...
        if account[0] != user_id or not secret.verify_password(password,
                                                               account[1]):
            raise TotoException(ERROR_USER_NOT_FOUND,
                                "Invalid user ID or password")
...
class RedisSessionCache(TotoSessionCache):
    def __init__(self, db):
        """``db`` must be an instance of ``redis.StrictRedis`` initialized to
        the target database.
        """
        self.db = db
    ...
    def store_session(self, session_data):
        ...
        self.db.setex(session_key, int(session_data['expires'] - time()),
                      TotoSession.dumps(session_data))
