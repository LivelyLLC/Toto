...
class PostgresSession(TotoSession):
    ...
    class PostgresAccount(TotoAccount):
        ...
        def _load_property(self, *args):
            return self._session._db.get('select ' + ', '.join(
                args) + ' from account where account_id = %s',
                                         (self._session.account_id))

        def _save_property(self, *args):
            self._session._db.execute('update account set ' + ', '.join(
                ['%s = %%s' % k for k in args]) + ' where account_id = %s', (
                                          [self[k] for k in args] + [
                                              self._session.account_id, ]))

        def __setitem__(self, key, value):
            if key != 'account_id':
                super(PostgresSession.PostgresAccount, self).__setitem__(key,
                                                                         value)
    ...
    def refresh(self):
        session_data = self._refresh_cache() or self.db.get(
            "select session.session_id, session.expires, session.state, "
            "       account.user_id, account.account_id "
            "from session "
            "join account on account.account_id = session.account_id "
    ...
    def save(self):
        ...
        if not self._save_cache():
            self._db.execute(
                "update session set state = %s where session_id = %s",
                (TotoSession.dumps(self.state), self.session_id))
...
class PostgresConnection(DBConnection):
    def create_tables(self):
        ...
        state bytea
            ...
            self.db.execute(
                'CREATE INDEX session_expires ON session USING BTREE (expires);')

    def __init__(self, host, port, database, username, password,
                 session_ttl=24 * 60 * 60 * 365, anon_session_ttl=24 * 60 * 60,
                 session_renew=0, anon_session_renew=0, min_connections=1,
                 max_connections=10):
        self.db = ThreadedConnectionPool(min_connections, max_connections,
    ...
    def create_account(self, user_id, password, additional_values={}, **values):
        ...
        self.db.execute("INSERT INTO account (" + ', '.join(
            [k for k in values]) + ") VALUES (" + ','.join(
            ['%s' for k in values]) + ")", [values[k] for k in values])

    def _load_uncached_data(self, session_id):
        return self.db.get(
            "select session.session_id, session.expires, session.state, "
            "       account.user_id, account.account_id "
            "from session join account on account.account_id = session.account_id "
            "where session.session_id = %s and session.expires > %s",
            (session_id, time()))
    ...
    def create_session(self, user_id=None, password=None, verify_password=True):
        ...
        account = user_id and self.db.get(
            ...
            (user_id,))
        if user_id and (not account or (verify_password and not
                secret.verify_password(password, account['password']))):
            raise TotoException(ERROR_USER_NOT_FOUND,
                                "Invalid user ID or password")
        session_id = PostgresSession.generate_id()
        expires = time() + (
            user_id and self.session_ttl or self.anon_session_ttl)
        session_data = {'user_id': user_id, 'expires': expires,
        ...
        if not self._cache_session_data(session_data):
            ...
            self.db.execute(
                "insert into session (account_id, expires, session_id) "
                "             values (%s, %s, %s)",
    ...
    def retrieve_session(self, session_id, hmac_data=None, data=None):
        ...
        if user_id and data and hmac_data != base64.b64encode(
                hmac.new(str(user_id), data, hashlib.sha1).digest()):
            raise TotoException(ERROR_INVALID_HMAC, "Invalid HMAC")
        expires = time() + (
            user_id and self.session_renew or self.anon_session_renew)
        if session_data['expires'] < expires:
            session_data['expires'] = expires
            if not self._cache_session_data(session_data):
                self.db.execute(
                    "update session set expires = %s where session_id = %s",
                    (session_data['expires'], session_id))
        session = PostgresSession(self.db, session_data, self._session_cache)
        session._verified = True
        return session

    def remove_session(self, session_id):
        self.db.execute("delete from session where session_id = %s",
                        (session_id,))

    def clear_sessions(self, user_id):
        user_id = user_id.lower()
        self.db.execute(
            "delete from session using session "
            "join account on account.account_id = session.account_id "
