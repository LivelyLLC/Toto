...
class MongoDBSession(TotoSession):
    ...
    class MongoDBAccount(TotoAccount):
        def _load_property(self, *args):
            return self._session._db.accounts.find_one(
                {'user_id': self._session.user_id}, {a: 1 for a in args})

        def _save_property(self, *args):
            self._session._db.accounts.update(
                {'user_id': self._session.user_id},
                {'$set': {k: self[k] for k in args}})
    ...
    def refresh(self):
        session_data = self._refresh_cache() or self._db.sessions.find_one(
    ...
    def save(self):
        ...
        if not self._save_cache():
            self._db.sessions.update({'session_id': self.session_id}, {
                '$set': {'state': TotoSession.dumps(self.state)}})
...
class MongoDBConnection(DBConnection):
    def _ensure_indexes(self):
        ...
        if not 'session_id' in session_indexes:
            self.db.sessions.ensure_index('session_id', unique=True,
                                          name='session_id')
    ...
    def __init__(self, host, port, database, session_ttl=24 * 60 * 60 * 365,
                 anon_session_ttl=24 * 60 * 60, session_renew=0,
                 anon_session_renew=0):
    ...
    def _load_uncached_data(self, session_id):
        return self.db.sessions.find_one(
            {'session_id': session_id, 'expires': {'$gt': time()}})
    ...
    def create_session(self, user_id=None, password=None, verify_password=True):
        ...
        if user_id and (not account or (
                    verify_password and not secret.verify_password(password,
                                                                   account[
                                                                       'password']))):
        ...
        if not self._cache_session_data(session_data):
            self.db.sessions.remove(
                {'user_id': user_id, 'expires': {'$lt': time()}})
    ...
    def retrieve_session(self, session_id, hmac_data=None, data=None):
        ...
        if session_data['expires'] < expires:
            ...
            if not self._cache_session_data(session_data):
                self.db.sesions.update({'session_id': session_id}, {
                    '$set': {'expires': session_data['expires']}})
        session = MongoDBSession(self.db, session_data, self._session_cache)
        session._verified = True
        return session
    ...
    def change_password(self, user_id, password, new_password):
        ...
        if not account or not secret.verify_password(password,
            ...
            raise TotoException(ERROR_USER_NOT_FOUND,
                                "Invalid user ID or password")
        self.db.accounts.update({'user_id': user_id}, {
            '$set': {'password': secret.password_hash(new_password)}})
